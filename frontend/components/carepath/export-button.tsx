...
<body>
  ...
  <div class="section">
    <div class="section-header">
      ...
      <span class="section-title">Recurring Tasks & Reminders</span>
    </div>
    <div class="section-body">
      <ul>
        ${result.recurring_tasks.map(t => `<li><strong>${escHtml(t.title)}</strong> (${escHtml(t.frequency)}): ${escHtml(t.description)}</li>`).join("")}
      </ul>
    </div>
  </div>
  ` : ""}
